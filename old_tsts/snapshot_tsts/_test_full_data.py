...
def test_full_data_view(snapshot):
    data = sorted(
        PartSerializer(Part.objects.all(), many=True).data,
        key=lambda item: (item["part"], item["year"]),
    )
    for d in data:
        del d["id"]
...
def test_all_parameters_view(snapshot):
    client = APIClient()
    user = get_user_model().objects.create_user("test@test.com", "testpass")
...
def request_with_test_client(year, param, part, breakdowns):
    ...
    payload = {"year": year, "param": param, "part": part, "breakdowns": breakdowns}
...
def test_aggregated_data_view(snapshot, year, param, part, breakdowns):
    ...
    snapshot.assert_match(
        yaml.dump(json.loads(res.content)),
